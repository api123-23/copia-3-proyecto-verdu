...
- **Fix error GE + opciones compresor (aplicado, sin commitear):**
  ...
  - `types.ts`: nuevo tipo `OkBajoAlto`; `ValoresBase.inst_electrica` y `carroceria` pasan de `SiNo` a `OkMal`; `InformeCompresor` saca `aceite_unidad` del Omit y la sobreescribe con `OkBajoAlto` (intersección).
  - `SeccionValores.tsx`: `aceite_unidad` del compresor usa `OPCIONES_NIVEL` (Ok/Bajo/Alto, `esCompresor` ya definido) y motocompresor conserva `OPCIONES_SI_NO`; `tiempo_y_delta` pasa de `OPCIONES_OK_BAJO` a `OPCIONES_NIVEL`. (opciones Ok/Bajo/Alto ya no requieren constante nueva).
  - Verificado: build OK (39 recursos precacheados), lint 0 errores (mismos 5 warnings). Pendiente: reejecutar `schema.sql` en Supabase, deploy y probar guardado de GE/compresor en celular.
